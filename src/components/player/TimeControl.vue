<template>
  <div class="controls" v-bind="$attrs" :class="{ active: isActive }">
    ...
    />
    <time class="text-white ml-4">{{ TotalTime }}</time>
  </div>
</template>
...
const calculateTime = (duration: number) => {
  ...
  const computedSecond = seconds > 10 ? seconds : `0 ${seconds}`

  return `${minutes}: ${computedSecond}`
}
const currentTime = computed(() => calculateTime(props.currentVideoPosition))
const TotalTime = computed(() => calculateTime(props.videoDuration))
...
<style scoped>
.time-track {
  @apply outline-none rounded-lg self-center opacity-70 cursor-pointer h-[8px] w-[90%];
